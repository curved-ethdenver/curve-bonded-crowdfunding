...
const NonStandardTokenRegistry = artifacts.require(
  "NonStandardTokenRegistry.sol"
);
...
contract("BancorConverter", accounts => {
  before(async () => {
    ...
    connectorToken = await TestERC20Token.new(
      "ERC Token 1",
      "ERC1",
      1000000000
    );
    connectorToken2 = await TestNonStandardERC20Token.new(
      "ERC Token 2",
      "ERC2",
      2000000000
    );
    connectorToken3 = await TestERC20Token.new(
      "ERC Token 3",
      "ERC2",
      1500000000
    );
  ...
  it("verifies the converter data after construction", async () => {
    ...
    let isSupported = await contractFeatures.isSupported.call(
      converter.address,
      featureWhitelist
    );
    assert(isSupported);
  ...
  it("should throw when a non owner attempts to update a connector", async () => {
    ...
    try {
      await converter.updateConnector(
        connectorToken.address,
        200000,
        false,
        0,
  ...
  it("verifies that the correct connector balance is returned regardless of whether virtual balance is set or not", async () => {
    ...
    let connectorToken = await TestERC20Token.new(
      "ERC Token 1",
      "ERC1",
      100000
    );
    ...
    connectorBalance = await converter.getConnectorBalance.call(
      connectorToken.address
    );
  ...
  it("verifies that getReturn returns a valid amount", async () => {
    let converter = await initConverter(accounts, true);
    let returnAmount = (await converter.getReturn.call(
      connectorToken.address,
      tokenAddress,
      500
    ))[0];
  ...
  it("verifies that getReturn returns the same amount as getPurchaseReturn when converting from a connector to the token", async () => {
    ...
    let purchaseReturnAmount = (await converter.getPurchaseReturn.call(
      connectorToken.address,
      500
    ))[0];
  ...
  it("verifies that getReturn returns the same amount as getSaleReturn when converting from the token to a connector", async () => {
    ...
    let saleReturnAmount = (await converter.getSaleReturn.call(
      connectorToken.address,
      500
    ))[0];
    assert.isNumber(returnAmount.toNumber());
  ...
  it("verifies that getReturn returns the same amount as buy -> sell when converting between 2 connectors", async () => {
    ...
    let purchaseRes = await converter.convert(
      connectorToken.address,
      tokenAddress,
      500,
      1
    ...
    let saleRes = await converter.convert(
      tokenAddress,
      connectorToken2.address,
      purchaseAmount,
      1
  ...
  it("verifies that Conversion event returns conversion fee after buying", async () => {
    ...
    assert(
      purchaseRes.logs.length > 0 && purchaseRes.logs[0].event == "Conversion"
    );
  ...
  it("should throw when attempting to get the return with identical from/to addresses", async () => {
    ...
    try {
      await converter.getReturn.call(
        connectorToken.address,
        connectorToken.address,
        500
      );
  ...
  it("verifies that convert returns a valid amount", async () => {
    let converter = await initConverter(accounts, true);
    await connectorToken.approve(converter.address, 500);
    let res = await converter.convert(
      connectorToken.address,
      tokenAddress,
      500,
      1
  ...
  it("verifies balances after buy", async () => {
    ...
    let connectorTokenPrevBalance = await connectorToken.balanceOf.call(
      accounts[0]
    );
    ...
    let connectorTokenNewBalance = await connectorToken.balanceOf.call(
      accounts[0]
    );
    assert.equal(
      connectorTokenNewBalance.toNumber(),
      connectorTokenPrevBalance.minus(500).toNumber()
    );

    let tokenNewBalance = await token.balanceOf.call(accounts[0]);
    assert.equal(
      tokenNewBalance.toNumber(),
      tokenPrevBalance.plus(purchaseAmount).toNumber()
    );
  });
  ...
  it("verifies balances after sell", async () => {
    ...
    assert.equal(
      connectorTokenNewBalance.toNumber(),
      connectorTokenPrevBalance.plus(saleAmount).toNumber()
    );

    let tokenNewBalance = await token.balanceOf.call(accounts[0]);
    assert.equal(
      tokenNewBalance.toNumber(),
      tokenPrevBalance.minus(500).toNumber()
    );
  });
  ...
  it("verifies that getReturn returns the same amount as getCrossConnectorReturn when converting between 2 connectors", async () => {
    ...
    let returnAmount2 = (await converter.getCrossConnectorReturn.call(
      connectorToken.address,
      connectorToken2.address,
      500
    ))[0];
  ...
  it("verifies that getCrossConnectorReturn returns the same amount as convert between 2 connectors", async () => {
    let converter = await initConverter(accounts, true);
    let returnAmount = (await converter.getCrossConnectorReturn.call(
      connectorToken.address,
      connectorToken2.address,
      500
    ))[0];
    ...
    await connectorToken.approve(converter.address, 500);
    let convertRes = await converter.convert(
      connectorToken.address,
      connectorToken2.address,
      500,
      1
  ...
  it("verifies that fund updates the virtual balance correctly", async () => {
    ...
    let prevConnector3Balance = await converter.getConnectorBalance.call(
      connectorToken3.address
    );
    ...
    let connector3Balance = await converter.getConnectorBalance.call(
      connectorToken3.address
    );
    assert.equal(
      connector3Balance.toNumber(),
      prevConnector3Balance.plus(Math.floor(token3Amount)).toNumber()
    );
  });
  ...
  it("verifies that fund gets the correct connector balance amounts from the caller", async () => {
    ...
    let prevConnector1Balance = await converter.getConnectorBalance.call(
      connectorToken.address
    );
    let prevConnector2Balance = await converter.getConnectorBalance.call(
      connectorToken2.address
    );
    let prevConnector3Balance = await converter.getConnectorBalance.call(
      connectorToken3.address
    ...
    let connector1Balance = await converter.getConnectorBalance.call(
      connectorToken.address
    );
    let connector2Balance = await converter.getConnectorBalance.call(
      connectorToken2.address
    );
    let connector3Balance = await converter.getConnectorBalance.call(
      connectorToken3.address
    ...
    assert.equal(
      connector1Balance.toNumber(),
      prevConnector1Balance.plus(Math.floor(token1Amount))
    );
    assert.equal(
      connector2Balance.toNumber(),
      prevConnector2Balance.plus(Math.floor(token2Amount))
    );
    assert.equal(
      connector3Balance.toNumber(),
      prevConnector3Balance.plus(Math.floor(token3Amount))
    );
    ...
    await connectorToken.transfer(accounts[0], token1Balance, {
      from: accounts[9]
    });
    await connectorToken2.transfer(accounts[0], token2Balance, {
      from: accounts[9]
    });
    await connectorToken3.transfer(accounts[0], token3Balance, {
      from: accounts[9]
    });
  });
  ...
  it("verifies that liquidate updates the virtual balance correctly", async () => {
    ...
    assert.equal(
      connector3Balance.toNumber(),
      prevConnector3Balance.minus(Math.floor(token3Amount)).toNumber()
    );
  });
  ...
  it("should have the registry point to itself after initialization", async () => {
    ...
    let registryAddress = await contractRegistry.getAddress.call(
      contractRegistryId
    );
